...
def execute_sql_query(connection, sql, phototype):
    try:
        ...
        # Initialize a counter to keep track of processed records
        processed_records = 0
        ...
        for row in rows:
            for column_name in column_names:
                if column_name == "data":
                    ...
                    imageProcessingErrors = json_data.get('imageProcessingErrors')  # Extract the 'img128' attribute
            ...
            # Increment the processed_records counter
            processed_records += 1

            # Check if 1000 records have been processed
            if processed_records % 1000 == 0:
                print(f"Processed {processed_records} out of {len(rows)} records")

        # After the loop, print a final status message
        print(f"Finished processing {processed_records} out of {len(rows)} records")

        # Define the CSV file name
        csv_file = phototype + "_not_found.csv"

        # Define the CSV field names (headers)
        field_names = ["photo_id", "expedition", "parent_identifier", "img128", "imageProcessingErrors"]
